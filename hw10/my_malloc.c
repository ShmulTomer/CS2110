...
static metadata_t *find_right(metadata_t *freed_block) {
    ...
    return (uintptr_t) ((uint8_t*) (freed_block + 1) + freed_block->size) == (uintptr_t) (freed_block -> next) ? freed_block -> next : NULL;
}
...
static metadata_t *split_block(metadata_t *block, size_t size) {
    ...
    metadata_t *res = (metadata_t *) ((uint8_t*) (block + 1) + block -> size - size - TOTAL_METADATA_SIZE);
    res -> size = size;
...
static void add_to_addr_list(metadata_t *block) {
    ...
    // adding to the free list ignoring merges
    ...
    while (curr) {
        ...
        if ((uintptr_t) curr < (uintptr_t) block && (uintptr_t) (curr -> next) > (uintptr_t) block) {
...
void *my_malloc(size_t size) {
    ...
    if (!best || best -> size < size) {
        metadata_t *newS = (metadata_t *)my_sbrk(SBRK_SIZE);
        if ((uintptr_t) newS == (uintptr_t) (-1)) {
            my_malloc_errno = OUT_OF_MEMORY;
            return NULL;
        }
        newS -> next = NULL;
        newS -> size = SBRK_SIZE - TOTAL_METADATA_SIZE;
        add_to_addr_list(newS);
    ...
    if (best -> size == size) {
        ...
        return (best + 1);
    } else if (best -> size >= size + TOTAL_METADATA_SIZE + 1) {
        //split
        metadata_t *right = split_block(best, size);
        remove_from_addr_list(right);
        return (right + 1);
    } else {
        // just return the bigger block
        remove_from_addr_list(best);
        return (best + 1);
    }
...
void my_free(void *ptr) {
    ...
    ptr = (metadata_t *) ((uint8_t*) ptr - TOTAL_METADATA_SIZE);
...
void *my_calloc(size_t nmemb, size_t size) {
    ...
    metadata_t *newB = my_malloc(nmemb * size);
    if (!newB) {
        return NULL;
    }

    memset(newB, 0, nmemb * size);
