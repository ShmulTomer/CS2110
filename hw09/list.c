...
static User *create_user(char *name, UserType type, UserUnion data)
{
    ...
    if (type == STUDENT) {
        struct student *tempS = (struct student *) malloc(sizeof(struct student));
        ...
        if (create_student(data.student.num_classes, data.student.grades, tempS) == 1) {
            free(tempS);
            free(temp);
            return NULL;
        }
        if (name) {
            char *nameStr = (char *) malloc(sizeof(char) * (strlen(name) + 1));
            if (nameStr == 0) {
                free(nameStr);
                free(tempS);
                free(temp);
        ...
        temp -> data.student = *tempS;
        temp -> type = STUDENT;
        free(tempS);
        return temp;
    } else {
        struct instructor *tempI = (struct instructor *) malloc(sizeof(struct instructor));
        if (tempI == 0) {
            free(tempI);
            free(temp);
            return NULL;
        }
        if (create_instructor(data.instructor.salary, tempI) == 1) {
            free(tempI);
            free(temp);
            return NULL;
        }
...
static Node* create_node(char *name, UserType type, UserUnion data)
{
    ...
    User *temp = create_user(name, type, data);
...
int num_passing_all_classes(LinkedList *list, int *dataOut)
{
    ...
    while (temp) {
        if (temp -> data -> type == STUDENT && temp -> data -> data.student.num_classes > 0) {
            int add = 1;
            for (int i = 0; i < temp -> data -> data.student.num_classes; i++) {
                if (temp -> data -> data.student.grades[i] < 60) {
                    add = 0;
                    break;
                }
            }
...
int get_average_salary(LinkedList *list, double *dataOut)
{
    ...
    while (temp) {
        if (temp -> data -> type == INSTRUCTOR) {
            sum += temp -> data -> data.instructor.salary;
            count++;
        }
        temp = temp -> next;
    }
